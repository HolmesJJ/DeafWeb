...
namespace Project20181209.Encryption
{
    public class RSAHelper
    {
        ...
        // 创建 RSAHelper 的一个对象
        private static RSAHelper RSAHelperInstance = new RSAHelper(RSAType.RSA2, Encoding.UTF8, RSAHelper.privateKey, RSAHelper.publicKey);
        // 用来加锁，防止多线程访问类时，会产生多个实例
        private static object _lock = new object();

        // 获取唯一可用的对象
        public static RSAHelper getInstance()
        {
            if (RSAHelperInstance == null)
            {
                lock (_lock)
                {
                    if (RSAHelperInstance == null)
                    {
                        RSAHelperInstance = new RSAHelper(RSAType.RSA2, Encoding.UTF8, RSAHelper.privateKey, RSAHelper.publicKey);
                    }
                }
            }
            return RSAHelperInstance;
        }
